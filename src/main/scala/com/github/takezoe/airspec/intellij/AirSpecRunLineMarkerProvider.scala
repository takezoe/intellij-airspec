...
class AirSpecRunLineMarkerProvider extends ScalaTestRunLineMarkerProvider {
  ...
  override def getInfo(element: PsiElement): RunLineMarkerContributor.Info = {
    ...
    element match {
      case IsAirSpecTestElement(td, tm) => buildInfo(td, tm)
      case _                            => null /* SIGH */
    }
  }
  ...
  private def iconFor(url: String, project: Project, isClass: Boolean): Icon = {
    ...
    def defaultIcon = if (isClass) TestState.Run_run else TestState.Run
